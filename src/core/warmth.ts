import { cosine, embedText, type StoredVectors } from "./engine.js";
import type { EngineConfig, WarmthConfig } from "./config.js";
import type { LinkGraph } from "./graph.js";

export interface WarmthSignal {
  title: string;
  score: number;
  source: "embedding" | "graph" | "both";
}
...
export function mergeWarmthScores(
  similarities: SimilarityEntry[],
  ...
  for (const title of mergedTitles) {
    ...
    const finalScore =
      (1 - config.graph_weight) * embeddingScore +
      config.graph_weight * graphScore;

    if (finalScore < noiseFloor) continue;

    const hasSeed = seeds.has(title);
    const hasGraph = graphScore > 0;
    const source: WarmthSignal["source"] = hasSeed && hasGraph
      ? "both"
      : hasSeed
        ? "embedding"
        : "graph";
...
export class WarmthService {
  private cache: WarmthCache | null = null;

  constructor(private readonly embedder: WarmthEmbedder = embedText) {}
  ...
  async scan(
    ...
    if (
      this.cache &&
      this.cache.vectorCount === storedVectors.size &&
      !detectSurprise(
        contextEmbedding,
